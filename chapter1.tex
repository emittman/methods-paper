...
We can see that BNP assigns low probabilities ($< 0.25$) to more genes for every type of heterosis. For LPH, BNP tends toward lower probabilites than found in \citet{landau2016high}. In the case of HPH, BNP estimates more genes with high probabilities of heterosis ($>0.75$) than Landau. Somewhat curiously, there seem to be more probabilities in the second column than would be consistent with the surrounding cells. It was stated in \citet{landau2016high} that this is likely an artifact of the hierarchical model in the presence of null genes.

While these results are somewhat ambiguous --- we do not know which model is more correct --- these tables highlight the impact that distributional assumptions can have on the inferences we obtain.

\iftoggle{thesis}{
  Because of the large number of genes, there were memory storage issues related to how many samples were actually stored when running our chains. This presented a trade-off: thinning reduces the effective number of samples, so it is generally something we want to avoid. One potential solution is to approximate distributions of interest by their first and second moments. Computation of these moments requires minimal storage, as running means can be used to update these after each iteration of the MCMC chain.
  
Figure \ref{compare-cis} shows full posterior distributions for  $\beta_g$ for a random sample of four genes and contrasts that with a normal approximation matching the first and second posterior moments. The quality of the moment-based approximation varies from gene to gene. In first two rows, we see that, the normal approximation is not very accurate due to the multimodal posterior. On the other hand, it is fairly accurate in the last two rows. Notably, the accuracy seems to be associated with the intercept paramters, meaning that when the overall expression level is higher, the moment-based approximation is more satisfactory. The lack of agreement for the genes with lower expression can be understood if we consider that  the ambiguity at lower expression levels leads to increased shrinkage. Due to the complexity in shrinkage distribution, this shrinkage can be directed in more than one direction, leading to the multimodality that we see. On the other hand, as expression levels increase, the ability to distiguish one gene from another increases, leading to smoother posterior distributions.

\begin{figure}
\includegraphics[width=\textwidth]{sample_genes_cis}
\caption{Posterior distributions for a random subset of genes.}
\includegraphics[width=\textwidth]{sample_genes_expression}
...
\footnotesize
\centering
\caption{Two-way tables comparing the posterior probabilities of different types of gene heterosis for all genes in the Paschold (2012) data set produced by BNP and Landau (2016).}
\label{bayes-compare}
...
& & [0,0.25] & (0.25,0.5] & (0.5,0.75] & (0.75,1] \\ 
  \hline
\multirow{4}{*}{BNP} & [0,0.25] & 27595 & 4359 & 784 & 158 \\ 
  & (0.25,0.5] & 1029 & 741 & 306 & 105 \\ 
  & (0.5,0.75] & 287 & 273 & 160 & 115 \\ 
  & (0.75,1] &  83 & 529 & 107 & 190 \\ 
   \bottomrule
\end{tabular}
\\[.5cm]
...
& & [0,0.25] & (0.25,0.5] & (0.5,0.75] & (0.75,1] \\ 
  \hline
\multirow{4}{*}{BNP} & [0,0.25] & 25648 & 4626 & 816 & 154 \\ 
  & (0.25,0.5] & 1428 & 754 & 382 & 158 \\ 
  & (0.5,0.75] & 633 & 500 & 282 & 184 \\ 
  & (0.75,1] & 207 & 604 & 188 & 257 \\ 
   \hline
\end{tabular}
\\[.5cm]
