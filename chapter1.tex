...
Comparing AUC across the ten simulations (Figure \ref{ss2-roc}), we see that, in most cases, DESeq2 provides better rankings without the empirical Bayes prior. The AUCs for the other three methods are comparable, with small differences depending on the effect type and the threshold. Overall, it appears that BNP and DESeq2 (unshrunk) have an edge over edgeR and that BNP may have a slight advantage over the other methods when the threshold is set fairly high.
